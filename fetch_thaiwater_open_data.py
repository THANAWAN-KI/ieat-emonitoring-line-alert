...
STORM_URL = "https://api-v3.thaiwater.net/api/v1/thaiwater30/public/storm_data"
FORECAST_URL = "https://api-v3.thaiwater.net/api/v1/thaiwater30/public/rain7day_forecast"
TWA_HOME_URL = "https://twa.thaiwater.net/th"
TWA_BASIC_MAP_URL = "https://twa.thaiwater.net/th/map/basic"
TWA_FLOOD_CURRENT_URL = "https://twa.thaiwater.net/th/map/flash-flood/water-level/overall"
TWA_FLOOD_WARNING_URL = "https://twa.thaiwater.net/th/map/flash-flood/warning-48h/overall/0"
TWA_RAIN_RISK_3DAY_URL = "https://www.thaiwater.net/risk-rainfall-3-day-forecast"
FLOOD_WATCH_URL = (
    "https://www.thaiwater.net/proxy/riskrainfall48h.php?file="
    "https://api.hii.or.th/v2/4UQaYnf0Bx4fXPYyCdDRbqHyXH9Ixvd2nVUjaN1cLBY="
...
def fetch_national_warning() -> dict[str, Any]:
    ...
    warning: dict[str, Any] = {
        "storms": [],
        "forecast_24h": {},
        "flood_watch_24h": {"province_count": 0, "provinces": [], "areas": []},
        "twa": {
            "home_url": TWA_HOME_URL,
            "basic_map_url": TWA_BASIC_MAP_URL,
            "current_flood_url": TWA_FLOOD_CURRENT_URL,
            "warning_24_48h_url": TWA_FLOOD_WARNING_URL,
            "rain_risk_3day_url": TWA_RAIN_RISK_3DAY_URL,
        },
        "errors": [],
    }
    try:
        payload = get_json(STORM_URL)
        for storm in payload.get("data", []):
            track = storm.get("storm_data") or []
            latest = track[-1] if track else {}
            warning["storms"].append(
                {
                    "name": storm.get("storm_name") or latest.get("storm_name") or "ไม่ระบุชื่อ",
                    "lat": number(latest.get("storm_lat")),
                    "lon": number(latest.get("storm_long")),
                    "wind_knot": number(latest.get("storm_wind")),
                    "observed_at": latest.get("storm_datetime") or "",
                }
            )
    except Exception as exc:
        warning["errors"].append(f"storm: {type(exc).__name__}: {exc}")
    try:
        payload = get_json(FORECAST_URL)
        items = payload.get("data", {}).get("pre_rain", {}).get("data", [])
        if items:
            item = items[0]
            token = item.get("media_path")
            warning["forecast_24h"] = {
                "valid_at": item.get("media_datetime") or "",
                "image_url": (
                    "https://api-v3.thaiwater.net/api/v1/thaiwater30/shared/image"
                    f"?_csrf=&image={urllib.parse.quote(str(token), safe='')}"
                    if token else ""
                ),
                "source": item.get("refer_source") or "",
            }
    except Exception as exc:
        warning["errors"].append(f"forecast: {type(exc).__name__}: {exc}")
    try:
        payload = get_json(FLOOD_WATCH_URL)
        areas = payload.get("area") or []
        provinces = sorted(
            {
                str(row.get("province") or row.get("province_name") or "").strip()
                for row in areas
                if str(row.get("province") or row.get("province_name") or "").strip()
            }
        )
        warning["flood_watch_24h"] = {
            "updated_at": payload.get("date") or "",
            "province_count": len(provinces),
            "provinces": provinces,
            "areas": areas[:100],
        }
    except Exception as exc:
        warning["errors"].append(f"flood_watch: {type(exc).__name__}: {exc}")
    return warning
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        ...
        "sources": [
            ...
            {"name": "ThaiWater ข้อมูลพายุ", "url": STORM_URL},
            {"name": "ThaiWater พยากรณ์ฝน", "url": FORECAST_URL},
            {"name": "ThaiWater พื้นที่เฝ้าระวังน้ำท่วม 24 ชั่วโมง", "url": FLOOD_WATCH_URL},
            {"name": "TWA พื้นที่เฝ้าระวังน้ำท่วม 24/48 ชั่วโมง", "url": TWA_FLOOD_WARNING_URL},
            {"name": "TWA พื้นที่น้ำท่วมปัจจุบัน", "url": TWA_FLOOD_CURRENT_URL},
            {"name": "TWA แผนที่พื้นฐาน", "url": TWA_BASIC_MAP_URL},
            {"name": "ThaiWater ความเสี่ยงจากฝนสะสมล่วงหน้า 3 วัน", "url": TWA_RAIN_RISK_3DAY_URL},
    ...
    try:
        ...
        stations = rain + water
        national_warning = fetch_national_warning()
        stations.sort(
            ...
            result.update(
                {
                    ...
                    "national_warning": previous.get("national_warning", {}),
